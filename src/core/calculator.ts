...
export class CalculatorCore {
  ...
  private calculate(expression: string): number {
    ...
    try {
      ...
      // Fix octal number issue by removing leading zeros from numbers
      // This prevents JavaScript from interpreting numbers like 09966 as octal
      // Also handle cases where numbers start with 0 but are not followed by other digits
